...
st.set_page_config(
    page_title="India Mutual Fund & ETF Theme Screener",
    page_icon="📊",
    layout="wide"
)

# Title & Description
st.title("🇮🇳 India Thematic Screener & Portfolio Planner")
st.markdown("""
...
for idx, row in top_themes.iterrows():
    ...
    with card_cols[idx]:
        st.markdown(f"""
        <div style="background-color: #f1f5f9; padding: 20px; border-radius: 12px; border-left: 5px solid #1e3a8a; height: 100%;">
            <span style="font-size: 11px; font-weight: bold; color: #2563eb; text-transform: uppercase;">Rank #{idx+1} Theme</span>
            <h3 style="margin-top: 5px; color: #1e3a8a;">{theme_name}</h3>
            <p style="font-size: 13px; color: #475569;">{desc_val}</p>
            ...
            <h4 style="margin-top: 2px; color: #1e293b;">{fund['name']}</h4>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 15px; font-size: 12px;">
                <div><strong>AUM:</strong> {fund['aum']}</div>
                <div><strong>Expense Ratio:</strong> {fund['expense']}</div>
                <div style="grid-column: span 2;"><strong>3-Year CAGR:</strong> {fund['r3y']}</div>
            </div>
            <div style="margin-top: 15px; font-weight: bold; font-size: 13px;">
                Tactical Call: {action_status} (Score: {score_val}/10)
            </div>
        </div>
